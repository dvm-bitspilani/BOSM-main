...
let stars = "";
const star_count_fact = Math.random() * 0.1 + 0.2;
let window_height = window.innerHeight;
let window_width = window.innerWidth;
let star_count = window_height * window_width * star_count_fact  / 1000;
const add_stars = () => {
	stars = "";
	for (let i = 0; i < star_count; i++) {
		...
		if (class_int < 0.5) {
			stars = `${stars}\n
<div class="star-type1 star" style="left: ${offset_x * window_width}px; top: ${offset_y * window_height}px"></div>`;
		} else {
			stars = `${stars}\n
<div class="star-type2 star" style="left: ${offset_x * window_width}px; top: ${offset_y * window_height}px"></div>`;
		}
	}
